...
uniform float kd;
uniform float ks, q;
uniform float reflectivity; // Fator de mistura entre Phong e o Cubemap
...
void main()
{
    ...
    // Parcela da reflexão especular (completada)
    vec3 V = normalize(cameraPos - fragPos);
    vec3 R_light = reflect(-L, N);
    float spec = pow(max(dot(V, R_light), 0.0), q);
    vec3 specular = ks * spec * lightColor;
    
    vec4 texColor = texture(texBuffer, texcoord);
    vec4 phongColor = (vec4(ambient, 1.0) + vec4(diffuse, 1.0) + vec4(specular, 1.0)) * texColor;

    // --- ENVIRONMENT MAPPING (CUBEMAP) ---
    // Vetor de incidência (da câmera para o fragmento)
    vec3 I = normalize(fragPos - cameraPos);
    // Vetor de reflexão
    vec3 R_env = reflect(I, N);
    // Amostra a cor do céu usando o vetor de reflexão
    vec4 skyboxColor = vec4(texture(skybox, R_env).rgb, 1.0);

    // --- INTEGRAÇÃO ---
    // Mistura a cor calculada por Phong com a cor do Cubemap usando a variável de refletividade
    color = mix(phongColor, skyboxColor, reflectivity);
}
)glsl";

//--------------------
const GLchar *vShaderSkybox = R"glsl(#version 450
...
const GLchar *fShaderSkybox = R"glsl( #version 450
out vec4 FragColor;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
        ...
        // Configuração da refletividade (0.0 = apenas Phong, 1.0 = espelho perfeito)
        float reflectivity = 0.4f; // Você pode alterar este valor ou vinculá-lo a uma tecla
        glUniform1f(glGetUniformLocation(shaderID, "reflectivity"), reflectivity);

        // Ativa e faz o bind da textura 2D do modelo na Unidade 0
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, m.texID);
        glUniform1i(glGetUniformLocation(shaderID, "texBuffer"), 0);
        //-------------------------------------------- Chamada de desenho do objeto

        // Ativa e faz o bind da textura do Cubemap na Unidade 1
